...
const rows = JSON.parse(await fs.readFile('outputs/keizai_rows.json', 'utf8'));
...
const guideValues = [
  ['24_kakomonn_keizai.pdf PDFゲーム用データ', 'PDF + Excel モード'],
  ['対象PDF', '24_kakomonn_keizai.pdf'],
  ['科目', '経済学・経済政策'],
  ['収録年度', '令和4年度、令和3年度、令和2年度、令和元年度、平成30年度、平成29年度'],
  ['収録ステージ数', rows.length],
  ['PDF総ページ数', 218],
  ['問題シート', 'A列は問題番号です。B列以降は回答範囲と正解の組を最大20小問まで管理します。'],
  ['複数設問', '設問1、設問2がある問題は、ゲーム画面に複数の入力欄を表示します。'],
...
const fileName = '24_kakomonn_keizai_pdf_game.xlsx';
const output = await SpreadsheetFile.exportXlsx(workbook);
await output.save(path.join(outputDir, fileName));

for (const [sheetName, range, imageName] of [
  ['問題', 'A1:G12', '24_kakomonn_keizai_questions_preview.png'],
  ['ページ情報', 'A1:I12', '24_kakomonn_keizai_pages_preview.png'],
  ['使い方', `A1:B${guideValues.length}`, '24_kakomonn_keizai_guide_preview.png'],
]) {
...
for (const range of ['問題!A1:G10', 'ページ情報!A1:I8', `使い方!A1:B${guideValues.length}`]) {
